...
import { SUBJECTS, FAST_TRACK_DURATIONS } from '../constants';
...
const FastTrackHubView: React.FC<Props> = ({ user, progress, language, onBack, onUpdateProgress }) => {
  ...
  const durations = FAST_TRACK_DURATIONS.map(mins => ({
    ...
    <div className="max-w-6xl mx-auto py-12 px-4 animate-fadeIn">
      ...
      {configuringSubject && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-xl animate-fadeIn">
          <div className="w-full max-w-2xl bg-white dark:bg-slate-900 rounded-[3rem] shadow-2xl border border-white/20 overflow-hidden flex flex-col max-h-[85vh]">
            ...
            <div className="p-10 space-y-8 overflow-y-auto custom-scrollbar">
              <section>
                <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] mb-6">Select Intensive Duration</h4>
                  ...
                  {durations.map(d => (
                    ...
                    >
                      <div className="flex flex-col items-center gap-2 mb-2 text-center">
                        ...
                        <span className="font-black dark:text-white text-xs">{d.value}m</span>
                      </div>
                      <span className="text-rose-500 font-black text-[8px] uppercase tracking-widest text-center w-full">{t('setup')} →</span>
                    </button>
                  ))}
                </div>
              </section>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[3.5rem] p-10 md:p-16 border-b-8 border-rose-500 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-8">
          <header className="text-center md:text-left">
            <div className="inline-block px-4 py-1.5 bg-rose-500/10 text-rose-500 rounded-full text-[10px] font-black uppercase tracking-widest border border-rose-500/20 mb-6">
              ⚡ {t('fastTrackLab')}
            </div>
          ...
          <div className="pt-12">
            <div className="flex gap-8 mb-8 border-b border-gray-100 dark:border-slate-800">
               <button 
                onClick={() => setActiveTab('A-P')}
                className={`pb-4 px-2 text-xs font-black uppercase tracking-widest transition-all relative ${activeTab === 'A-P' ? 'text-rose-500 border-b-4 border-rose-500' : 'text-gray-400 hover:text-gray-600'}`}
               >
                <span>{t('foundationPath')}</span>
                <span className="block text-[8px] mt-1 font-bold">{t('fastTrackAPDuration')}</span>
               </button>
               <button 
                onClick={() => setActiveTab('Q-T')}
                className={`pb-4 px-2 text-xs font-black uppercase tracking-widest transition-all relative ${activeTab === 'Q-T' ? 'text-rose-500 border-b-4 border-rose-500' : 'text-gray-400 hover:text-gray-600'}`}
               >
                <span>{t('advancedPath')}</span>
                <span className="block text-[8px] mt-1 font-bold">{t('fastTrackQTDuration')}</span>
               </button>
            </div>
               ...
               {filteredSubjects.map(sub => {
                 ...
                 return (
                   <button 
                    ...
                    className={`p-6 rounded-[2rem] border-2 transition-all flex items-center justify-between text-left group ${isFast ? 'border-rose-500 bg-rose-50/5 shadow-xl' : 'border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-800/50 hover:border-rose-500/40'}`}
                   >

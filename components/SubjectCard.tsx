    ...
    >
      <header className="flex justify-between items-start mb-10 relative z-10">
        <div className="relative group">
           <div className={`w-24 h-24 bg-slate-950/10 rounded-[2rem] flex items-center justify-center text-6xl shadow-inner transition-all duration-500 group-hover:scale-110 group-hover:rotate-6`}>
              <span className="drop-shadow-xl">{subject.icon}</span>
           </div>
           {isFastTrack && (
        ...
        <div className="flex flex-col items-end gap-3">
           <span className={`px-6 py-2 rounded-2xl text-[11px] font-black uppercase tracking-widest border-2 transition-all ${isHovered ? `bg-slate-950 text-white border-slate-950 shadow-2xl` : `bg-slate-950/20 text-slate-950 border-slate-950/30`}`}>
              LVL {level}
           </span>
           <button onClick={(e) => { e.stopPropagation(); onOpenSpecialization(); }} className="px-4 py-1.5 bg-slate-950/10 text-[9px] font-black text-slate-950 uppercase tracking-widest rounded-xl hover:bg-slate-950 hover:text-white transition-all">
              DNA Mapping
           </button>
        </div>
      </header>
      ...
      <div className="flex-1 space-y-4 relative z-10 mb-10">
        <h3 className={`text-4xl font-black ${themeConfig.text} tracking-tighter leading-none uppercase`}>{subject.name}</h3>
        <p className={`text-base ${themeConfig.text} opacity-80 font-bold leading-relaxed line-clamp-3 italic`}>"{subject.description}"</p>
      </div>

      <div className="space-y-4 relative z-10 mb-10">
         <div className={`flex justify-between items-center text-[10px] font-black ${themeConfig.text} opacity-60 uppercase tracking-widest`}>
            <span>Grid Mastery</span>
            <span>{lessonNumber}/12 Chapters</span>
         </div>
         <div className="h-4 w-full bg-slate-950/10 rounded-full overflow-hidden shadow-inner p-1">
            <div className="h-full bg-slate-950 rounded-full transition-all duration-1000" style={{ width: `${(lessonNumber / 12) * 100}%` }}></div>
         </div>
      </div>

      <footer className="mt-auto pt-8 border-t-2 border-slate-950/10 flex items-center justify-between relative z-10">
        <div className="text-left">
           <p className={`text-[10px] font-black ${themeConfig.text} opacity-50 uppercase tracking-widest mb-1`}>Official Node</p>

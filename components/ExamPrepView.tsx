...
import { Subject, Language, User, LessonContent } from '../types';
...
const ExamPrepView: React.FC<Props> = ({ user, subject, language, onBack, onComplete }) => {
  ...
  const boards = [
    ...
    { name: 'Custom Institutional Prep', weeks: 8, system: 'Global Modular' }
  ];
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-8 animate-fadeIn">
        ...
        <div className="text-center">
          ...
          <p className="text-gray-500 font-bold">Aligning darewast mastery to {activeBoard} curricula...</p>
        </div>
      </div>
    );
  }
  ...
  if (prepContent) {
    return (
      <div className="max-w-4xl mx-auto py-12 px-4 animate-fadeIn">
        <button onClick={() => setPrepContent(null)} className="mb-8 text-gray-400 font-bold flex items-center gap-2 group transition-all">
          <span className="group-hover:-translate-x-1 transition-transform">←</span> Back to Boards
        </button>
        ...
        <div className="space-y-10">
          ...
          <section className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] shadow-xl border border-gray-100 dark:border-slate-800">
             <div className="flex items-center gap-3 mb-8">
                ...
                <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest">Tactical Insight Lab</h3>
             </div>
             <div className="grid gap-4">
                {prepContent.tacticalTips.map((tip, i) => (
                  <div key={i} className="flex items-start gap-4 p-4 bg-pink-50 dark:bg-pink-900/10 rounded-2xl border border-pink-100 dark:border-pink-900/20">
                     ...
                     <p className="text-pink-800 dark:text-pink-300 font-bold text-sm leading-relaxed">{tip}</p>
                  </div>
                ))}
             </div>
          </section>
          ...
          <section className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] shadow-xl border border-gray-100 dark:border-slate-800">
             <div className="flex items-center gap-3 mb-6">
                <span className="w-10 h-1 bg-dare-teal rounded-full"></span>
                <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest">Focus Theory</h3>
             </div>
             <p className="text-xl font-medium text-gray-700 dark:text-gray-300 leading-relaxed">{prepContent.explanation}</p>
          </section>

          <section className="space-y-8">
             <h3 className="text-xs font-black text-gray-400 uppercase tracking-[0.4em] text-center">Board Simulation Drills</h3>
                 ...
                 <div key={i} className={`p-10 rounded-[3rem] border-2 transition-all ${isChecked ? (isCorrect ? 'border-emerald-500 bg-emerald-50/10' : 'border-rose-500 bg-rose-50/10') : 'border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900'}`}>
                    ...
                    <div className="grid gap-3">
                       {ex.options?.map(opt => (
                         <button 
                           ...
                           className={`p-5 text-left rounded-2xl font-bold transition-all ${isChecked ? (opt === ex.correctAnswer ? 'bg-emerald-500 text-white shadow-lg' : (opt === userAns ? 'bg-rose-500 text-white' : 'opacity-20')) : 'bg-gray-50 dark:bg-slate-800 hover:bg-gray-100 text-gray-600 dark:text-gray-300'}`}
                         >
                           {opt}
                         </button>
                       ))}
                    ...
                    {isChecked && (
                      <div className="mt-8 pt-8 border-t border-gray-100 dark:border-slate-800 animate-fadeIn">
                         <p className="text-[10px] font-black uppercase text-gray-400 mb-2">Strategy Explanation</p>
                         <p className="text-sm font-bold text-gray-600 dark:text-gray-400 italic">"{ex.explanation}"</p>
                      </div>
                    )}
                 </div>
               );
          ...
          <button 
            onClick={onComplete}
            className="w-full py-8 bg-gradient-to-r from-dare-purple to-pink-500 text-white rounded-[3rem] font-black text-2xl shadow-2xl hover:scale-[1.02] active:scale-95 transition-all"
          >
            Conclude Prep Session 🎯
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto py-12 px-4 animate-fadeIn">
      <button onClick={onBack} className="mb-8 text-gray-400 font-bold flex items-center gap-2 group transition-all">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[3.5rem] p-12 border border-gray-100 dark:border-slate-800 shadow-2xl overflow-hidden relative">
        ...
        <p className="text-gray-500 text-xl mb-12 leading-relaxed max-w-2xl font-medium">
          The darewast Exam engine covers <span className="text-dare-purple font-black">all curricula, methods, and systems</span> worldwide. Choose a target board to begin tactical synthesis.
        </p>
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          {boards.map(board => (
            <button
              key={board.name}
              onClick={() => handleStartPrep(board.name)}
              className="p-8 rounded-[2.5rem] text-left border-2 border-gray-50 dark:border-slate-800 hover:border-dare-purple hover:bg-dare-purple/5 bg-white dark:bg-slate-900 shadow-sm hover:shadow-xl transition-all group"
            >
        ...
        <div className="mt-16 p-10 bg-slate-50 dark:bg-slate-800/50 rounded-[3rem] text-center border-2 border-dashed border-gray-200 dark:border-slate-700">
          <p className="text-gray-500 font-bold max-w-md mx-auto leading-relaxed">
            Preparations adapt to <span className="text-dare-purple font-black">Montessori, Kumon, and Traditional</span> learning methods automatically based on your academic DNA.
          </p>
        </div>
      </div>
    </div>

...
const LessonView: React.FC<Props> = ({ subject, language, level, lessonNumber, user, progress, initialLesson, onComplete, onBack, onUpdateUser, onUpdateProgress }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto py-12 px-6 animate-fadeIn grid lg:grid-cols-12 gap-12 bg-slate-950">
      {isCelebrating && (
        <div className="fixed inset-0 z-[400] bg-slate-950 flex items-center justify-center p-6 animate-fadeIn">
          <div className="max-w-4xl w-full bg-dare-gold rounded-[5rem] p-24 text-center shadow-2xl border-[10px] border-white/40 animate-scale-in relative overflow-hidden">
            ...
            <div className="space-y-6 mb-24">
              <h2 className="text-7xl md:text-9xl font-black text-slate-950 tracking-tighter leading-none uppercase">Node Anchored!</h2>
      ...
      <aside className="lg:col-span-3 space-y-10 lg:sticky lg:top-32 h-fit">
        <button onClick={onBack} className="w-full py-6 bg-dare-gold text-slate-950 font-black uppercase text-xs tracking-[0.3em] rounded-[2rem] border-4 border-white/20 hover:bg-slate-950 hover:text-white transition-all shadow-2xl">← Return Dashboard</button>
        
        <div className="bg-dare-teal p-10 rounded-[4rem] shadow-2xl border-4 border-white/20">
           <p className="text-[11px] font-black text-slate-950 uppercase tracking-[0.5em] mb-8">Metadata Path</p>
           <div className="space-y-6">
              <div className="p-6 bg-slate-950 text-white rounded-3xl border-4 border-white/10 shadow-inner">
      ...
      <main className="lg:col-span-9 space-y-20">
        <header className="bg-slate-900 p-12 md:p-24 rounded-[4.5rem] text-white shadow-2xl relative overflow-hidden border-4 border-white/10">
           <div className="absolute top-0 right-0 p-12 opacity-10 text-[15rem] rotate-12">{subject.icon}</div>
           <div className="relative z-10 space-y-12">
              <div className="flex flex-wrap gap-3">
                 {['Unified', 'Visual', 'Auditory', 'Reading', 'Kinesthetic'].map(s => (
                   <button key={s} onClick={() => setLearningStyle(s as any)} className={`px-6 py-3 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all ${learningStyle === s ? 'bg-dare-teal text-slate-950 shadow-2xl scale-110' : 'bg-slate-950 text-slate-500 border-2 border-white/10'}`}>{s}</button>
                 ))}
              </div>
              <h1 className="text-6xl md:text-9xl font-black tracking-tighter leading-[0.8] uppercase">{lesson?.title}</h1>
           </div>
        </header>

        <section className="bg-dare-purple p-12 md:p-24 rounded-[5rem] shadow-2xl border-[6px] border-white/20 relative overflow-hidden group">
           <div className="absolute top-0 right-0 p-16 opacity-10 text-[12rem] font-black text-white">LOGIC</div>
           <h3 className="text-xs font-black text-white uppercase tracking-[0.6em] mb-16 opacity-60">{t('concept')}</h3>
           <div className="text-4xl md:text-6xl font-black text-white leading-[1.1] italic mb-20 tracking-tighter">"{renderExplanationWithPronunciation(currentAdaptation || '')}"</div>
           
           {lesson?.timelineSteps && (
             <div className="space-y-10 pt-20 border-t-4 border-white/10">
                {lesson.timelineSteps.map((step, idx) => (
                  <div key={idx} onClick={() => setActiveTimelineStep(idx)} className={`p-12 rounded-[3.5rem] cursor-pointer transition-all border-[6px] ${activeTimelineStep === idx ? 'border-white bg-slate-950 shadow-2xl scale-[1.03]' : 'border-transparent bg-white/5 opacity-50 hover:opacity-100'}`}>
                    <div className="flex items-center gap-6 mb-6">
                       <span className={`text-sm font-black ${activeTimelineStep === idx ? 'text-dare-teal' : 'text-white'} uppercase tracking-[0.5em]`}>Node 0{idx+1}</span>
                       <h5 className="font-black uppercase tracking-tighter text-white text-4xl">{step.title}</h5>
                    </div>
                    {activeTimelineStep === idx && <p className="text-slate-300 font-bold text-3xl leading-relaxed animate-fadeIn">{renderExplanationWithPronunciation(step.detail)}</p>}
                  </div>
                ))}
             </div>
           )}
        ...
        <section className="space-y-16">
           ...
           {lesson?.exercises.map((ex, i) => (
             <div key={i} className={`p-14 md:p-20 rounded-[5rem] bg-slate-900 border-[6px] border-white/5 transition-all ${checkedIndices.has(i) ? 'border-dare-teal shadow-[0_40px_80px_rgba(83,205,186,0.3)] bg-slate-950' : 'shadow-2xl'}`}>
               <div className="flex justify-between items-start mb-16">
                 <span className="text-9xl font-black text-white/5 leading-none">{(i+1).toString().padStart(2, '0')}</span>
                 <button onClick={() => handleToggleHint(i)} className={`text-[11px] font-black uppercase px-8 py-3 rounded-2xl transition-all flex items-center gap-3 ${visibleHints.has(i) ? 'bg-dare-gold text-slate-950 shadow-2xl scale-110' : 'bg-slate-800 text-white'}`}>
                    <span>{visibleHints.has(i) ? 'Dismiss' : 'Neural Hint'}</span>
                    <span className="text-xl">💡</span>
                 </button>
               </div>
               <h4 className="text-4xl md:text-6xl font-black text-white mb-20 leading-[1.1] tracking-tighter uppercase">{renderExplanationWithPronunciation(ex.question)}</h4>
               
               {visibleHints.has(i) && (
                 <div className="mb-20 p-12 bg-dare-gold text-slate-950 rounded-[3.5rem] border-4 border-white/40 animate-fadeIn shadow-2xl relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-8 opacity-10 text-8xl font-black rotate-12">HINT</div>
                    <div className="flex items-start gap-6 relative z-10">
                       <span className="text-6xl">💡</span>
                       <div className="space-y-2">
                          <p className="text-[11px] font-black uppercase tracking-widest opacity-60">Axiomatic Insight Node</p>
                          <p className="text-2xl font-black italic leading-tight">"{ex.hint || 'Analyze the core logic modeling of the previous node.'}"</p>
                       </div>
                    </div>
                 </div>
               )}
               ...
               <div className="grid gap-6">
                  {ex.options?.map(opt => (
                    <button 
                      ...
                      className={`p-12 text-left rounded-[3rem] border-4 transition-all font-black text-3xl flex justify-between items-center group ${checkedIndices.has(i) && userAnswers[i] === opt ? 'bg-dare-teal text-slate-950 border-white shadow-2xl scale-[1.02]' : (checkedIndices.has(i) ? 'opacity-10 border-white/5' : 'border-white/10 text-white bg-slate-950 hover:border-dare-teal hover:bg-slate-900')}`}
                    >
        ...
        <button onClick={() => setIsCelebrating(true)} className="w-full py-16 bg-dare-teal text-slate-950 rounded-[5rem] font-black text-5xl shadow-2xl hover:scale-[1.02] active:scale-95 transition-all group overflow-hidden relative border-[8px] border-white/30 uppercase tracking-tighter">
          Finalize Academic Node <span className="inline-block group-hover:translate-x-8 transition-transform ml-8">🏆</span>
        </button>
      </main>
